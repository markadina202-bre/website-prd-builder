...
- M-034: Baris "hint fast-forward" di output push = PUSH DITOLAK (bukan sukses). Verifikasi push HANYA via `git ls-remote` vs `git rev-parse HEAD` — `git status` kosong + tanpa upstream = bukti palsu. Branch sesi solo → `--force-with-lease` aman bila lokal superset remote (cek diff per file dulu).
